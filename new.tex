...
\def\eloop{
  ...
  \pscustom{%
   ...
   \blockend{\getlength{\the\mylen}}{0.411}{0.5}{0.3}{\strip@pt\xshift}{\strip@pt\yshift}
   \closepath
   \closedshadow
  }
	\rput[0](!\strip@pt\xshift\space \strip@pt\mypos\space){
		\uput[0](!0 -0.711 2 div){\color{white}\textsf{\bfseries \expandafter\csname text\expandafter\romannumeral\mycount\endcsname}}
		}
...
\traceon
	\bloop{répéter \psframebox{23} fois }
\traceoff
	\block{mouvement}{s'orienter à \field*{90}}
  % \bloop{répéter \field{4} fois }
  \block{stylo}{effacer tout ! }
